...
def parse_args():
    parser = argparse.ArgumentParser(description="Prepare DeepfakeTIMIT dataset for DeepGuard.")
    parser.add_argument(
        "--dataset-root",
        type=str,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="data/processed/deepfaketimit",
...
def discover_deepfaketimit_videos(dataset_root: Path, max_videos: int | None, seed: int):
    """
    Discover all DeepfakeTIMIT videos with optional subsampling.
    DeepfakeTIMIT has structure:
    - higher_quality/  (128x128 models)
    - lower_quality/   (64x64 models)
    Videos are .avi files organized by subject folders.
    """
